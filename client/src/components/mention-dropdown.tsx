...
export function MentionDropdown({ query, onSelect, visible }: MentionDropdownProps) {
  ...
  return (
    <div className="absolute z-50 bg-background border rounded-lg shadow-lg w-64 max-h-48 overflow-y-auto">
      {users.slice(0, 5).map((user: any) => (
        ...
        >
          <Avatar className="w-7 h-7">
            ...
            <AvatarFallback className="text-xs bg-primary/10 text-primary">{user.displayName.charAt(0)}</AvatarFallback>
          </Avatar>
